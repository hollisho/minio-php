...
### StsClientIntegrationTest
测试 STS 完整使用流程和实际场景：
- **完整 STS 工作流程**：服务端获取临时凭证 → 客户端使用临时凭证操作
- **不同权限策略场景**：只读权限、用户目录权限、文件大小限制等
- **临时凭证生命周期**：短期凭证 vs 长期凭证
- **前端直传场景**：后端生成临时凭证 → 前端直接上传到 MinIO

**STS 使用场景：**
1. 前端/移动端直传文件到 MinIO（避免文件经过后端服务器）
2. 为不同用户分配不同权限的临时访问
3. 第三方应用临时访问授权
4. 提高安全性（临时凭证自动过期）
